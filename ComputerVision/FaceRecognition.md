...
便可以作为特征通过计算欧式距离进行人脸校验。上面得到的分数向量能够进一步得到改善，通过在欧式空间里面使用”triplet loss”的方式进行训练。其实这里所说使用”triplet loss”的方式进行训练，是对特征的进一步精炼，使用的”triplet loss”学习方非常的常见，是度量学习里面的一种，下面具体讲一下这个特征再学习过程。
